...
public class ThanksAgent extends Agent {
	...
	public final static String GREETINGS = "POZDROWIENIA";
	public final static String ANSWER = "ODPOWIEDZ";
	public final static String THANKS = "DZIEKI";
	...
	protected void setup() {
		...
		try {
			...
			System.out.println(getLocalName()+" ZAREJESTROWANY DO DF");
		...
		if (IAmTheCreator) {
			...
			try {
				...
				t1.start();
				System.out.println(getLocalName()+" STWORZONO I URUCHOMIONO AGENTA:"+t1AgentName + " W CONTAINERZE "+container.getContainerName());
			} catch (Exception any) {
				any.printStackTrace();
			}
			...
			try {
				...
				// fire-up the agent
				t2.start();
				System.out.println(getLocalName()+" STWORZONO I URUCHOMIONO AGENTA:"+t2AgentName + " W CONTAINERZE "+ac.getContainerName());
			} catch (Exception e2) {
				e2.printStackTrace();
			}
			...
			send(msg);
			System.out.println(getLocalName()+" WYSLIJ WIADOMOSC DO "+t1AgentName+" I "+t2AgentName);
			...
			public void action() {
				...
				if (msg != null) {
					if (GREETINGS.equalsIgnoreCase(msg.getContent())) {
						...
						System.out.println(myAgent.getLocalName()+" ODEBRANIE POWITALNEJ WIADOMOSCI OD "+msg.getSender().getLocalName());
						ACLMessage reply = msg.createReply();
						reply.setContent(ANSWER);
						myAgent.send(reply);
						System.out.println(myAgent.getLocalName()+" WYSLANE ODPOWIEDZI NA WIADOMOSCI");
					} 
					else if (ANSWER.equalsIgnoreCase(msg.getContent())) {
						// if an ANSWER to a greetings message is arrived 
						// then send a THANKS message
						System.out.println(myAgent.getLocalName()+" OTRZYMANO WIADOMOSCI OD"+msg.getSender().getLocalName());
						ACLMessage replyT = msg.createReply();
						replyT.setContent(THANKS);
						myAgent.send(replyT);
						System.out.println(myAgent.getLocalName()+" WYSLANE PODZIEKOWANIA W WIADOMOSCI");
						answersCnt++;
						if (answersCnt == 2) {
						...
						System.out.println(myAgent.getLocalName()+" OTRZYMAL PODZIEKOWANIA W  WIADOMOSCI OD "+msg.getSender().getLocalName());
					}
					else {
						System.out.println(myAgent.getLocalName()+" Otrzymano nieoczekiwanA wiadomoSC od "+msg.getSender().getLocalName());
					}					
				}
				else {
	...
	protected void takeDown() {
		...
		try {
			...
			System.out.println(getLocalName()+" WYREJESTROWANIE Z DF");
		} catch (FIPAException e) {
			e.printStackTrace();
		}
	}
